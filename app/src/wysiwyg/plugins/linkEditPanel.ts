...
class LinkEditPanelView extends EditPanel {
      ...
      if (linkMark) {
        ...
        try {
          ...
          if (domNode.nodeType === Node.ELEMENT_NODE) {
            ...
            linkElement =
              (element.closest('.image-link') as HTMLElement) ||
            ...
            linkElement =
              (domNode.parentElement.closest('.image-link') as HTMLElement) ||
  ...
  private createDialogContent(dialog: HTMLElement) {
    ...
    const { linkUrl = '', target = null, rel = null } = this.state.linkNode.attrs;

    const currentUrl = this.state.tempChanges.linkUrl ?? linkUrl;
    const currentText =
      this.state.tempChanges.linkText ?? (this.state.linkElement?.textContent || '');
  ...
  private bindDialogEvents(dialog: HTMLElement) {
    ...
    const targetPresetBtns = dialog.querySelectorAll(
      '.target-presets .preset-btn',
    ) as NodeListOf<HTMLButtonElement>;
    const relPresetBtns = dialog.querySelectorAll(
      '.rel-presets .preset-btn',
    ) as NodeListOf<HTMLButtonElement>;
  ...
  private saveChanges() {
    ...
    if (!node) return;
    const start = this.state.linkPos;
    ...
    if (
      'linkText' in this.state.tempChanges &&
      typeof this.state.tempChanges.linkText !== 'undefined'
    ) {
