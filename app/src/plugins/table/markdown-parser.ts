...
export const tableMarkdownParsers: CustomParserMap = {
  // @ts-expect-error
  ...
  tableCell(node: MergedTableCellMdNode, { entering }) {
    ...
    if (entering) {
      ...
      const [colspan, contentWithColspan] = getSpanInfo(content, '@cols', '@rows');
      const [rowspan, contentWithRowspan] = getSpanInfo(contentWithColspan, '@rows', '@cols');

      node.stringContent = contentWithRowspan;
